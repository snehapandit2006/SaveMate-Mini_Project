...
export default function App() {
  ...
  // Reset the add goal dialog state when switching tabs
  useEffect(() => {
    if (activeTab !== 'goals') {
  ...
  const renderActiveTab = () => {
    switch (activeTab) {
      case 'home':
        return <Dashboard onEmergencyMode={() => setIsEmergencyMode(true)} userData={userData} onNavigateToGoals={handleNavigateToGoals} />;
      case 'goals':
        return <Goals openAddDialog={openAddGoalDialog} />;
      case 'budget':
        return <Budget />;
      case 'rewards':
        return <Rewards />;
  ...
  if (currentScreen === 'loginTypeSelection') {
    return (
      <>
        <LoginTypeSelection onSelectType={handleLoginTypeSelect} />
        <Toaster />
      </>
    );
  }
