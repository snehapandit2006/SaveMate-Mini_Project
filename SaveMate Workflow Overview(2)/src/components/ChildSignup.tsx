...
export default function ChildSignup({ onComplete, onBack }: ChildSignupProps) {
  ...
  const isFormValid = () => {
    const basicValidation = formData.firstName && 
           formData.lastName && 
           formData.email && 
           formData.phone && 
    ...
    if (basicValidation && formData.dateOfBirth) {
              ...
              <div>
                ...
                {calculatedAge !== null && (
                  <p className="text-xs text-gray-600 mt-1">
                    ...
                    {calculatedAge < 18 && (
                      <span className="text-red-600 ml-2">• You must be 18 or older to use SaveMate</span>
                    )}
                  </p>
                )}
                {calculatedAge !== null && calculatedAge < 18 && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-3 mt-2">
                    <p className="text-xs text-red-800">
                      <span className="font-semibold">Age Restriction:</span> SaveMate is only available for users aged 18 and above. Please come back when you're eligible.
                    </p>
                  </div>
                )}
              </div>
